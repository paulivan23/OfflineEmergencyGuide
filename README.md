...
Include emergency contact lists - This feature centralizes vital national and local hotline numbers with direct-dial capabilities to ensure users can reach professional help instantly during a crisis.

Share disaster preparedness tips - The objective is to equip users with proactive safety checklists and planning strategies for regional hazards like earthquakes and floods to minimize risk before an event occurs.
...
Step 2: Click Introduction, then click Create a project to get started.
 
Step 3: On our desktop, click the window button plus R; after that, the dialog box will appear on the screen, then type CMD or drop down click CMD and click ok in order for Command Prompt to show up.
   
Step 4: After the CMD appears, type cd Documents, then type mkdir my-mobile-app, then cd my-mobile-app, and lastly type node –v to check the version.
 ` 
Step 5: Go to websites after you click the "create a project" button, and copy and paste the terminal code to CMD to get started.
  
Step 6: After we run the last step, the result will show up like the photos below, and we will wait for 15 minutes to get our project ready.
  
Step 7: Go again to the Expo Docs website and set up the environment, then click Android device, which is where we would like to develop our app. And scan the QR code to install the Expo Docs app and set it up.
 
Step 8: Go back to the CMD to run our project type cd my-app and then code .(dot) to redirect to VS Code.
 
Step 9: In VS Code, open a new terminal and type npx expo start to generate a QR code to scan by the Expo Go app.

Step 10: After we scan the QR in step 9, it will redirect to this Expo Go app.


Firestore Collection, Field Overview and other related documents about our app "Offline Emergency Guide" are present in our 
